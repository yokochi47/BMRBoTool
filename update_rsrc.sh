...
if [ $# = 0 ] || [ $1 = "xsd" ] ; then
 ...
 java -cp $BMRBX_TOOL_HOME/xsd2pgschema.jar xsd2pgschema --xsd $XSD_SCHEMA --no-rel --hash-by SHA-1 > $DB_SCHEMA
# DB_SCHEMA=schema/mmcif_nmr-star.xsd-strict.schema
# java -cp $BMRBX_TOOL_HOME/xsd2pgschema.jar xsd2pgschema --xsd $XSD_SCHEMA > $DB_SCHEMA
 DB_SCHEMA=schema/mmcif_nmr-star.xsd-no_key.schema
 java -cp $BMRBX_TOOL_HOME/xsd2pgschema.jar xsd2pgschema --xsd $XSD_SCHEMA --no-key --no-doc-key --hash-by SHA-1 > $DB_SCHEMA
 JSON_SCHEMA=schema/mmcif_nmr-star.json
 java -cp $BMRBX_TOOL_HOME/xsd2pgschema.jar xsd2jsonschema --xsd $XSD_SCHEMA --col-json --discarded-doc-key-name entry_id > $JSON_SCHEMA
...
if [ $# = 0 ] || [ $1 = "owl" ] ; then
 ...
 ./bmrbx2owl.sh
 cd ..
 java -classpath ./owl-ann.jar:./extlibs/* OWL_ann --home .
